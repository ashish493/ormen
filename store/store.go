...
func NewInMemoryTaskStore() *InMemoryTaskStore {
	return &InMemoryTaskStore{
		Db: make(map[string]*sail.Sail),
	}
}

func (i *InMemoryTaskStore) Put(key string, value interface{}) error {
	t, ok := value.(*sail.Sail)
	if !ok {
		return fmt.Errorf("value %v is not a sail.Sail type", value)
	}
	i.Db[key] = t
	return nil
}
...
func NewInMemoryTaskEventStore() *InMemoryTaskEventStore {
	return &InMemoryTaskEventStore{
		Db: make(map[string]*sail.SailEvent),
	}
}

func (i *InMemoryTaskEventStore) Put(key string, value interface{}) error {
	e, ok := value.(*sail.SailEvent)
	if !ok {
		return fmt.Errorf("value %v is not a sail.SailEvent type", value)
	}
	i.Db[key] = e
	return nil
}
...
func (t *TaskStore) Put(key string, value interface{}) error {
	return t.Db.Update(func(tx *bolt.Tx) error {
		...
		buf, err := json.Marshal(value.(*sail.Sail))
		if err != nil {
			return err
		}
...
func (e *EventStore) Put(key string, value interface{}) error {
	return e.Db.Update(func(tx *bolt.Tx) error {
		...
		buf, err := json.Marshal(value.(*sail.SailEvent))
		if err != nil {
			return err
		}
